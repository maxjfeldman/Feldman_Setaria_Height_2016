...
def phenofront_parser(args):
  ...
  for row in csvfile:
    ...
    img_list = img_list[:-1]
    imgs = img_list.split(';')
    # Can do time filtering here I think
    img_datetime = data[colnames['timestamp']]
    img_datetime = img_datetime.replace(' ', '-')
    img_datetime = img_datetime.replace(':', '-')
    img_datetime = re.sub('\..*$', '', img_datetime)
    img_datetime = datetime.datetime.strptime(img_datetime, '%Y-%m-%d-%H-%M-%S')
    
    if args.dates:
    
      dates = args.dates.split('_')
      start_datetime = dates[0]
      start_datetime = datetime.datetime.strptime(start_datetime[:-3], '%Y-%m-%d-%H-%M-%S')
      end_datetime = dates[1]
      end_datetime = datetime.datetime.strptime(end_datetime[:-3], '%Y-%m-%d-%H-%M-%S')
    
      if img_datetime < start_datetime:
          continue
      if img_datetime > end_datetime:
          continue
